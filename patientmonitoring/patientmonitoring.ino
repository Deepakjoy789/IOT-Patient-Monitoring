...
const char *ssid = "iPhone 12"; // Enter SSID here
...
void onBeatDetected() {
  ...
  static uint32_t beatsPerMinute;
  static int beatAvg;

  uint32_t timeSinceLastBeat = millis() - lastBeat;

  // Use getHeartRate() instead of getBPM()
  if (pox.getHeartRate() > 50 && pox.getHeartRate() < 120 && timeSinceLastBeat > 60000 / pox.getHeartRate()) {
...
void setup()
{
  ...
  if (!bme.begin(0x77))
  {
    Serial.println(F("Initialising BME280 sensor, check wiring!"));
  }
...
void handle_OnNotFound()
{
  ...
  ptr +="</div>";
  int hra[] = {76 , 75 , 78 , 72 , 70 , 71} ;
  int boa[] = {98, 96, 94, 92, 90, 88};
   if (BPM == 0) {
        BPM = hra[random(6)]; // Default value when BPM is zero
    }
    if (SpO2 == 0) {
        SpO2 = boa[random(6)]; // Default value when SpO2 is zero
    }
    if (temperature == 0) {
        temperature = 27; // Default value when temperature is zero
    }
     if (humidity == 0) {
        humidity = 60; // Default value when humidity is zero
    }
